...
extern "C" {
	...
	{
		UCHAR reserved[0xC8];
		PVOID ExAcquireResourceSharedLite;  // + 0xc8
	};
	...
	static void GMonpFakeExQueueWorkItem( _Inout_ __drv_aliasesMem void *work_item, _In_ void *queue_type );

	DECLSPEC_NORETURN void GMonWaitForever( _In_ const AllRegisters *registers, _In_ ULONG_PTR stack_pointer );
...
#pragma alloc_text(INIT, GMonInstallPatchCallback)
	...
	_Use_decl_annotations_ void GMonTermination( )
	{
		PAGED_CODE( );
	}
	...
	{
		const auto pg_context = reinterpret_cast<PgContext *>(GMonIsPgExcutionContext( registers ));
		if (!pg_context)
		{
			return 0;
		}
		...
		auto epilogue_address = UtilMemMem( reinterpret_cast<void *>(guest_ip), 0x400, kDpcEpiloguePattern8_10, sizeof( kDpcEpiloguePattern8_10 ) );
		if (!epilogue_address)
		{
			// Try Win7 pattern if failed
			epilogue_address = UtilMemMem( reinterpret_cast<void *>(guest_ip), 0x400, kDpcEpiloguePattern7, sizeof( kDpcEpiloguePattern7 ) );
		}
		...
		{
			// Not. Likely to be in a main validation routine. Let the thread return to AsmWaitForever().
			return reinterpret_cast<ULONG_PTR>(AsmWaitForever);
		}
	}

	// Checks if the context have a reference to the PatchGuard context.
	_Use_decl_annotations_ static ULONG_PTR GMonIsPgExcutionContext( const GpRegisters *registers )
	{
		// clang-format off
		if (GMonpIsPgContext( registers->ax ))
		{
			return registers->ax;
		}
		if (GMonpIsPgContext( registers->bx ))
		{
			return registers->bx;
		}
		if (GMonpIsPgContext( registers->cx ))
		{
			return registers->cx;
		}
		if (GMonpIsPgContext( registers->dx ))
		{
			return registers->dx;
		}
		if (GMonpIsPgContext( registers->di ))
		{
			return registers->di;
		}
		if (GMonpIsPgContext( registers->si ))
		{
			return registers->si;
		}
		if (GMonpIsPgContext( registers->bp ))
		{
			return registers->bp;
		}
		if (GMonpIsPgContext( registers->r9 ))
		{
			return registers->r9;
		}
		if (GMonpIsPgContext( registers->r10 ))
		{
			return registers->r10;
		}
		if (GMonpIsPgContext( registers->r11 ))
		{
			return registers->r11;
		}
		if (GMonpIsPgContext( registers->r12 ))
		{
			return registers->r12;
		}
		if (GMonpIsPgContext( registers->r13 ))
		{
			return registers->r13;
		}
		if (GMonpIsPgContext( registers->r14 ))
		{
			return registers->r14;
		}
		if (GMonpIsPgContext( registers->r15 ))
		{
	...
	{
		const auto p_ExAcquireResourceSharedLite = g_gmonp_ExAcquireResourceSharedLite;
		...
		auto potential_ExAcquireResourceSharedLite = &pg_context->ExAcquireResourceSharedLite;
		...
		for (auto i = 0u; i < kMaxPointersToCheck; ++i)
		{
			...
			{
				if (*potential_ExAcquireResourceSharedLite == p_ExAcquireResourceSharedLite)
				{
					return true;
				}
			}
	...
	// ExQueueWorkItem().
	_Use_decl_annotations_ static void GMonpNeutralizePgContextForDpc( PgContext *pg_context )
	{
		auto potential_ExAcquireResourceSharedLite = &pg_context->ExAcquireResourceSharedLite;
	...
	_Use_decl_annotations_ static void GMonpFakeExQueueWorkItem( _Inout_ __drv_aliasesMem void *work_item, _In_ void *queue_type )
	{
		UNREFERENCED_PARAMETER( work_item );
		UNREFERENCED_PARAMETER( queue_type );

		HYPERPLATFORM_LOG_INFO_SAFE( "PatchGuard context has been detected and terminated." );
	}
...
#pragma warning(disable : 28167)
	_Use_decl_annotations_ void GMonWaitForever( const AllRegisters *registers, ULONG_PTR stack_pointer )
	{
...
#pragma warning(push)
		...
		{
			status = KeWaitForSingleObject( PsGetCurrentThread( ), Executive, KernelMode, FALSE, nullptr );
